...
export const registerQueueTools = (server, config) => {
    ...
    server.registerTool("queue_conclusion", {
        ...
        return Promise.resolve(renderTool({
            status: "queued",
    ...
    server.registerTool("queue_message", {
    ...
    server.registerTool("flush_conclusions", {
        ...
        try {
            ...
            return renderError(error instanceof Error ? error : new Error(String(error)));
        }
    })());
    server.registerTool("flush_messages", {
        ...
        try {
            ...
            const client = queue.getClient();
            await client.request("POST", `/workspaces/${args.workspace_id}/sessions/${args.session_id}/messages`, { messages: messages.map((m) => ({
                    content: m.content,
                    peer_id: m.peer_id,
                    metadata: m.metadata,
