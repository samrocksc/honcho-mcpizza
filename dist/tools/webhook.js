...
export const registerWebhookTools = (server, layer) => {
    server.registerTool("webhook_get_or_create", {
        ...
        const result = yield* client.request("POST", `/workspaces/${args.workspace_id}/webhooks`, { url: args.url });
        return renderTool(result);
    }).pipe(Effect.provide(layer), Effect.catchTag("HonchoClientError", (e) => Effect.succeed(renderError(e))))));
    server.registerTool("webhook_list", {
        ...
        const client = yield* HonchoClient;
        const result = yield* client.request("GET", `/workspaces/${args.workspace_id}/webhooks`);
        return renderTool(result);
    }).pipe(Effect.provide(layer), Effect.catchTag("HonchoClientError", (e) => Effect.succeed(renderError(e))))));
    server.registerTool("webhook_delete", {
        ...
        const client = yield* HonchoClient;
        yield* client.request("DELETE", `/workspaces/${args.workspace_id}/webhooks/${args.endpoint_id}`);
        return renderTool({ status: "deleted" });
    }).pipe(Effect.provide(layer), Effect.catchTag("HonchoClientError", (e) => Effect.succeed(renderError(e))))));
    server.registerTool("webhook_test", {
        ...
        const client = yield* HonchoClient;
        const result = yield* client.request("GET", `/workspaces/${args.workspace_id}/webhooks/test`);
        return renderTool(result);
    }).pipe(Effect.provide(layer), Effect.catchTag("HonchoClientError", (e) => Effect.succeed(renderError(e))))));
};
