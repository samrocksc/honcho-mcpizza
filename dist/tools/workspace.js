...
export const registerWorkspaceTools = (server, layer) => {
    server.registerTool("workspace_get_or_create", {
        ...
        const result = yield* client.request("POST", "/workspaces", { id: args.id, metadata: args.metadata, configuration: args.configuration });
    ...
    server.registerTool("workspace_update", {
        ...
        const client = yield* HonchoClient;
        const result = yield* client.request("PUT", `/workspaces/${args.workspace_id}`, { metadata: args.metadata, configuration: args.configuration });
        return renderTool(result);
    }).pipe(Effect.provide(layer), Effect.catchTag("HonchoClientError", (e) => Effect.succeed(renderError(e))))));
    ...
    server.registerTool("workspace_search", {
        ...
        const client = yield* HonchoClient;
        const result = yield* client.request("POST", `/workspaces/${args.workspace_id}/search`, { query: args.query, limit: args.limit, filters: args.filters });
        return renderTool(result);
    }).pipe(Effect.provide(layer), Effect.catchTag("HonchoClientError", (e) => Effect.succeed(renderError(e))))));
    server.registerTool("workspace_queue_status", {
        ...
        if (args.observer_id)
            params.append("observer_id", args.observer_id);
        if (args.sender_id)
            params.append("sender_id", args.sender_id);
        if (args.session_id)
            params.append("session_id", args.session_id);
        const query = params.toString() ? `?${params.toString()}` : "";
        const result = yield* client.request("GET", `/workspaces/${args.workspace_id}/queue/status${query}`);
        return renderTool(result);
    }).pipe(Effect.provide(layer), Effect.catchTag("HonchoClientError", (e) => Effect.succeed(renderError(e))))));
    server.registerTool("workspace_schedule_dream", {
        ...
        const client = yield* HonchoClient;
        const result = yield* client.request("POST", `/workspaces/${args.workspace_id}/schedule_dream`, {
