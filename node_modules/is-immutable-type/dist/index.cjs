...
var typeMatchesSpecifier = require('ts-declaration-location');
    ...
    const patterns = specifier instanceof RegExp
        ...
        : typeof specifier === "string" || specifier.pattern === undefined
            ...
            : Array.isArray(specifier.pattern)
                ? specifier.pattern
                : [specifier.pattern];
    const ignoreNames = typeof specifier === "string" || specifier instanceof RegExp || specifier.ignoreName === undefined
        ? []
        : Array.isArray(specifier.ignoreName)
            ? specifier.ignoreName
            : [specifier.ignoreName];
    const ignorePatterns = typeof specifier === "string" || specifier instanceof RegExp || specifier.ignorePattern === undefined
        ? []
...
function defaultTypeMatchesPatternSpecifier(program, type, typeNode, include, exclude = []) {
    ...
    if (typeSymbolName !== null && typeSymbolName !== typeNameAlias && typeSymbolName !== typeNameName) {
        const testTypeSymbolName = (pattern) => typeof pattern === "string" ? pattern === typeSymbolName : pattern.test(typeSymbolName);
 ...
 */
function getTypeSymbolName(type) {
    const target = "target" in type ? type.target : type;
...
function isTypeReferenceWithTypeArguments(type) {
    return tsApiUtils.isTypeReference(type) && type.typeArguments !== undefined && type.typeArguments.length > 0;
}
 ...
 * applied or not.
 */
function getTypeImmutability(program, typeOrTypeNode, overrides = getDefaultOverrides(), useCache = true, maxImmutability = exports.Immutability.Immutable, typeMatchesPatternSpecifier = defaultTypeMatchesPatternSpecifier) {
...
function getTypeImmutabilityHelper(program, td, overrides, useCache, maxImmutability, typeMatchesPatternSpecifier) {
    ...
    const parameters = {
        ...
        cache,
        immutabilityLimits: {
            min: exports.Immutability.Mutable,
            max: maxImmutability,
        },
        typeMatchesPatternSpecifier,
    };
    const mut_stack = [createNewTaskState(td)];
    let mut_PreviousImmutability = exports.Immutability.Unknown;
    let mut_state;
    do {
        mut_state = mut_stack.pop();
        ...
        if (mut_state.immutability !== exports.Immutability.Calculating) {
...
function createNewTaskState(typeData) {
    return {
        ...
        immutability: exports.Immutability.Calculating,
    };
}
/**
 ...
 */
function taskTriage(parameters, mut_stack, mut_state) {
    ...
    cacheData(parameters.program, parameters.cache, mut_state.typeData, mut_state.immutability);
    if (tsApiUtils.isUnionType(mut_state.typeData.type)) {
        handleTypeUnion(parameters, mut_stack, mut_state);
        return;
    }
    if (tsApiUtils.isIntersectionType(mut_state.typeData.type)) {
        handleTypeIntersection(parameters, mut_stack, mut_state);
        return;
    }
    if (tsApiUtils.isConditionalType(mut_state.typeData.type)) {
    ...
    if (tsApiUtils.isObjectType(mut_state.typeData.type)) {
 ...
 */
function taskObjectTypeReference(mut_stack, mut_state) {
    mut_stack.push(createCheckDoneTaskState(mut_state, () => {
 ...
 */
function taskObjectIndexSignature(parameters, mut_stack, mut_state) {
    const [types, typeNodes] = tsApiUtils.isIntersectionType(mut_state.typeData.type)
        ? [mut_state.typeData.type.types, mut_state.typeData.typeNode?.types]
    ...
    mut_stack.push(createCheckDoneTaskState(mut_state, () => {
        ...
        const children = types.flatMap((type, index) => createIndexSignatureTaskStates(parameters, mut_state, ts.IndexKind.Number, getTypeData(type, typeNodes?.[index])));
        if (children.length > 0) {
            mut_stack.push(createChildrenReducerTaskState(mut_state, children, max), ...children);
        }
    }));
    const children = types.flatMap((type, index) => createIndexSignatureTaskStates(parameters, mut_state, ts.IndexKind.String, getTypeData(type, typeNodes?.[index])));
    if (children.length > 0) {
        mut_stack.push(createChildrenReducerTaskState(mut_state, children, max), ...children);
    }
}
 ...
 */
function taskApplyOverride(mut_state) {
    if ((mut_state.override.from <= mut_state.taskState.immutability &&
 ...
 */
function taskCheckDone(mut_state, immutability) {
    if (immutability !== exports.Immutability.Calculating) {
 ...
 */
function taskReduceChildren(mut_state) {
    mut_state.immutability = (mut_state.children[0]).immutability;
    for (let mut_index = 1; mut_index < mut_state.children.length; mut_index++) {
        mut_state.immutability = mut_state.childrenReducer(mut_state.immutability, mut_state.children[mut_index].immutability);
    }
}
/**
 * Handle a type we know is a union.
 */
function handleTypeUnion(parameters, mut_stack, mut_state) {
    const checker = parameters.program.getTypeChecker();
    ...
    const unionTypeNode = mut_state.typeData.typeNode === null ? null : findUnionTypeNode(checker, mut_state.typeData.typeNode);
 ...
 */
function findUnionTypeNode(checker, typeNode) {
 ...
 */
function handleTypeIntersection(parameters, mut_stack, mut_state) {
    handleTypeObject(parameters, mut_stack, mut_state);
}
/**
 * Handle a type we know is a conditional type.
 */
function handleTypeConditional(parameters, mut_stack, mut_state) {
    const checker = parameters.program.getTypeChecker();
    const children = [mut_state.typeData.type.root.node.trueType, mut_state.typeData.type.root.node.falseType].map((typeNode) => {
 ...
 */
function handleTypeFunction(mut_state) {
    mut_state.immutability = exports.Immutability.Immutable;
}
/**
 * Handle a type we know is a tuple.
 */
function handleTypeTuple(parameters, mut_stack, mut_state) {
    if (!mut_state.typeData.type.target.readonly) {
        mut_state.immutability = exports.Immutability.Mutable;
        return;
    }
    handleTypeArray(parameters, mut_stack, mut_state);
}
 ...
 */
function handleTypeArray(parameters, mut_stack, mut_state) {
    // It will have limits after being processed by `handleTypeObject`.
    const mut_stateWithLimits = mut_state;
    mut_stack.push(createCheckDoneTaskState(mut_stateWithLimits, () => {
 ...
 */
function handleTypeObject(parameters, mut_stack, mut_state) {
    // Add limits.
    const mut_stateWithLimits = mut_state;
    ...
    if (properties.length > 0) {
        for (const property of properties) {
            if (tsApiUtils.isPropertyReadonlyInType(mut_stateWithLimits.typeData.type, property.getEscapedName(), checker) ||
                // Ignore "length" for tuples.
                // TODO: Report this issue to upstream.
                (property.escapedName === "length" && checker.isTupleType(mut_stateWithLimits.typeData.type))) {
                continue;
            }
            ...
            if (declarations.length > 0) {
                if (declarations.some((declaration) => hasSymbol(declaration) && tsApiUtils.isSymbolFlagSet(declaration.symbol, ts.SymbolFlags.Method))) {
                    mut_stateWithLimits.limits.max = min(mut_stateWithLimits.limits.max, exports.Immutability.ReadonlyDeep);
                    continue;
                }
            ...
            mut_stateWithLimits.immutability = exports.Immutability.Mutable;
            return;
        }
    }
    const propertyNodes = new Map(mut_stateWithLimits.typeData.typeNode !== null &&
        tsApiUtils.hasType(mut_stateWithLimits.typeData.typeNode) &&
        ...
        ? mut_stateWithLimits.typeData.typeNode.type.members
            .map((member) => member.name === undefined || !tsApiUtils.hasType(member) || member.type === undefined
            ? undefined
    ...
    const children = properties
        .map((property) => {
        const propertyType = typeUtils.getTypeOfPropertyOfType(checker, mut_stateWithLimits.typeData.type, property);
        if (propertyType === undefined || (tsApiUtils.isIntrinsicType(propertyType) && propertyType.intrinsicName === "error")) {
            return null;
        }
    ...
    if (children.length > 0) {
        mut_stateWithLimits.limits.min = exports.Immutability.ReadonlyShallow;
 ...
 */
function handleTypeArguments(mut_stack, mut_state) {
    const children = mut_state.typeData.type.typeArguments.map((type) => createNewTaskState(getTypeData(type, undefined)));
    mut_stack.push(createChildrenReducerTaskState(mut_state, children, min), ...children);
}
/**
 * Handle a primitive type.
 */
function handleTypePrimitive(mut_state) {
    mut_state.immutability = exports.Immutability.Immutable;
}
/**
 * Create the task states for analyzing an object's index signatures.
 */
function createIndexSignatureTaskStates(parameters, mut_state, kind, typeData) {
    const checker = parameters.program.getTypeChecker();
    const indexInfo = checker.getIndexInfoOfType(typeData.type, kind);
    if (indexInfo === undefined) {
        mut_state.immutability = exports.Immutability.Unknown;
        return [];
    }
    if (parameters.immutabilityLimits.max <= exports.Immutability.ReadonlyShallow) {
        mut_state.immutability = exports.Immutability.ReadonlyShallow;
        return [];
    }
    if (indexInfo.isReadonly) {
        ...
        return [
            createChildrenReducerTaskState(mut_state, [{ immutability: exports.Immutability.ReadonlyShallow }, child], max),
            child,
        ];
    }
    mut_state.immutability = exports.Immutability.Mutable;
