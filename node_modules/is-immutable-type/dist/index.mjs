...
import { isIntrinsicErrorType, isTypeReference, isIntersectionType, isUnionType, isConditionalType, isObjectType, isPropertyReadonlyInType, isSymbolFlagSet, hasType, isIntrinsicType } from 'ts-api-utils';
...
function handleTypeObject(parameters, mut_stack, mut_state) {
    ...
    if (properties.length > 0) {
        for (const property of properties) {
            if (isPropertyReadonlyInType(mut_stateWithLimits.typeData.type, property.getEscapedName(), checker) ||
            ...
            if (declarations.length > 0) {
                if (declarations.some((declaration) => hasSymbol(declaration) && isSymbolFlagSet(declaration.symbol, ts.SymbolFlags.Method))) {
    ...
    const propertyNodes = new Map(mut_stateWithLimits.typeData.typeNode !== null &&
        ...
        ? mut_stateWithLimits.typeData.typeNode.type.members
            .map((member) => member.name === undefined || !hasType(member) || member.type === undefined
            ? undefined
